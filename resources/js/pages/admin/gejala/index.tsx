...
import { BreadcrumbItem, GejalaTypes } from '@/types';
import { Head, useForm } from '@inertiajs/react';
import { LoaderCircle, PenBox } from 'lucide-react';
import { useMemo, useState } from 'react';
interface GejalaIndexProps {
    gejala: GejalaTypes[];
...
export default function GejalaIndex({ gejala, breadcrumb, titlePage }: GejalaIndexProps) {
    const breadcrumbs: BreadcrumbItem[] = useMemo(
        () => (breadcrumb ? breadcrumb.map((item) => ({ title: item.title, href: item.href })) : []),
        [breadcrumb],
    );

    const { data, setData, post, put, processing, errors } = useForm<gejalaFormData>({
        id: null,
        nama: '',
    });
    ...
    const handleEdit = (id: number) => {
        if (id) {
            const gejalaTemp: GejalaTypes[] = gejala.filter((item) => item.id === id, []);
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={titlePage ?? 'Gejala'} />

            {/* Data */}
            <Card>
                <div className="container mx-auto px-4">
                    <div className="mb-4 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <h2 className="text-lg font-bold md:text-xl">Gejala Nutrisi Tanaman</h2>
                                            ...
                                            <TableCell>
                                                <div className="flex flex-row items-center gap-2">
                                                    ...
                                                    <DeleteConfirmationForm
                                                        title={`Hapus gejala ${item.id}`}
                                                        id={item.id}
                                                        url={route('admin.gejala.destroy', { gejala: item.id })}
            ...
            <Dialog open={isOpenDialog} onOpenChange={setIsOpenDialog}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>{editId ? `Edit` : `Tambah`} Gejala</DialogTitle>
                    </DialogHeader>
